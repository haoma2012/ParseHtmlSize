...
/**
 * Created by yangdehao@xiaoyouzi.com  on 2017/4/10 21:45
 */
...
public class CustomTagHandler implements Html.TagHandler {
    ...
    public void endSpan(String tag, Editable output, XMLReader xmlReader){
        ...
        if (!TextUtils.isEmpty(size)) {
            int fontSizePx = 16;
            if (null != mContext){
                fontSizePx = DeviceUtils.sp2px(mContext,Integer.parseInt(size));
            }
    ...
    private void analysisStyle(int startIndex,int stopIndex,Editable editable,String style){
        ...
        if (!TextUtils.isEmpty(fontSize)) {
            int fontSizePx = 16;
            if (null != mContext){
                fontSizePx = DeviceUtils.sp2px(mContext,Integer.parseInt(fontSize));
            }
